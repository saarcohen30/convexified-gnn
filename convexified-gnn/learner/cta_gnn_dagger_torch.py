...
class CTADAGGER(object):
    ...
    def gradient_step(self, batch, cx):
        """
    ...
    def initialize_online_learning(self, is_convex):
        ...
        self.online_actor = CxActor(n_s, n_a, hidden_layers, k, ind_agg, self.device).to(self.device)
        for i in range(self.actor.n_layers):
            if i == self.actor.n_layers - 1:
                self.online_actor.conv_layers[i].weight = torch.nn.Parameter(self.actor.conv_layers[i].weight.clone())
                continue
            if is_convex:
                A = self.actor.conv_layers[i].A_sum.clone()
