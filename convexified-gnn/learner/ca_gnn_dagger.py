...
from learner.state import MultiAgentState
...
from learner.cx_actor import CxActor
...
class CADAGGER(object):
    ...
    def __init__(self, device, args, k=None):  # , n_s, n_a, k, device, hidden_size=32, gamma=0.99, tau=0.5):
        ...
        # Define Networks
        self.actor = CxActor(n_s, n_a, hidden_layers, k, ind_agg, device).to(self.device)
    ...
    def select_action(self, state, is_test):
        ...
        mu = self.actor(state.delay_state, state.curr_gso, is_test)  # .to(self.device)
    ...
    def gradient_step(self, batch):
        ...
        curr_gso_batch = Variable(torch.cat(tuple([s.curr_gso for s in batch.state]))).to(self.device)
        delay_state_batch = Variable(torch.cat(tuple([s.delay_state for s in batch.state]))).to(self.device)
        actor_batch = self.actor(delay_state_batch, curr_gso_batch, False)
    ...
    def select_action_online(self, state):
        ...
        @return:
        """
        mu = self.online_actor(state.delay_state, state.curr_gso, True)  # .to(self.device)
        ...
        #     mu = self.actor_per_agent[i](state.delay_state, state.curr_gso, True)  # .to(self.device)
...
def train_CADAGGER(env, args, device):
    ...
    for i in range(n_train_episodes):
        ...
        state = MultiAgentState(device, args, env.reset(), prev_state=None)
        ...
        while not done:
            ...
            next_state = MultiAgentState(device, args, next_state, prev_state=state)
